...
![图 23 显性→隐性转换:SIC 工作过程总览](../files/sic-design/images/fig23_dr_overview.svg)
...
![图 24 输出级三态阻抗切换](../files/sic-design/images/fig24_dr_impedance.svg)

*图 24:三态阻抗切换机理——显性强制差分驱动;有源隐性以 75~133 Ω(RDIFF_act_rec)主动拉向隐性中点并阻尼反射;被动隐性回到高阻,保证多节点共享总线与显性覆盖仲裁。*

![图 25 反射波被有源隐性阻抗吸收](../files/sic-design/images/fig25_dr_reflection.svg)

*图 25:反射机理对比——无 SIC 时反射在开放端几乎全反射、来回振荡;有 SIC 时反射波进入 ≈100 Ω 有源隐性支路被吸收/阻尼,总线快速回到 0.5 V 以下。*

![图 26 采样点与振铃抑制窗口](../files/sic-design/images/fig26_dr_sampling.svg)

*图 26:判据——SIC 窗口必须让总线在最早采样点前回到 0.5 V 以下;无 SIC 时振铃反复越阈,在 RXD 上产生伪毛刺,导致位错误。*

!!! tip "一句话总结"
    SIC 的显隐转换 = **低阻显性 → 中阻有源隐性(主动拉回 + 阻尼反射)→ 高阻被动隐性**,所有窗口参数都是为了"在采样点前把总线可靠送回隐性"。
