...
>
> 本知识库面向 **模拟 IC 设计工程师**(嵌入式工程师为辅助读者),围绕「协议 → 物理层 → 收发器设计 → 测试认证」聚合权威标准、专利、论文、教材与厂商资料,并提供分角色学习路线、原创教程与项目实践笔记。建站动机:团队已流片一颗 CAN FD (SIC) 收发器芯片——这里沉淀的就是这条路上需要的一切。

## 站点数据

| 指标 | 数量 |
|------|-----:|
| 资源条目(标准/专利/论文/期刊/教材/厂商/工具/专题) | 115 |
| 公开 PDF 直接下载 | 45 |
| 设计参考(收发器模块参数设计要点) | 10 |
| 原创教程 | 12 |
| 知识库子域 / 知识点 | 8 / 22 |
| 术语词条 | 51 |
| 原创 SVG 技术图 + Mermaid 图 | 22 + 100+ |

## 设计参考(模拟 IC 工程师优先)

!!! tip "正在设计/流片 CAN FD / CAN SIC 收发器?"
    按模块查阅参数设计要点:规格基线 → 时序预算 → 输出级 → 接收比较器 → SIC 控制 → 振铃抑制 → ESD/总线保护 → 电源唤醒,最后用总表做流片前自查。

    - [设计参考总览](design/index.md) —— 设计流程地图与模块入口
    - [参数集与规格基线](design/parameter-sets.md) —— Set A/B/C + Annex A,先锁规格树
    - [参数设计要点总表](design/parameter-checklist.md) —— 全模块逐项对标 + 自查清单

## 优先入口:SIC 学习笔记

!!! tip "正在做或准备做 CAN SIC 收发器?从这里开始"
    **SIC 设计专题学习笔记**:原理 → 设计 → 测试三章整合为单文件网页,含侧边目录、章节导航、搜索过滤,可打印为 PDF。所有关键数值取自一手资料(ISO 11898-2:2024、TI SLLA581、NXP TJA1463 等),并标注出处。
    ...
    - [配套资料(SIC 设计专题资源分类)](resources/sic-design.md) —— 10 篇 CAN Newsletter / iCC 论文,含本地 PDF

## 按你的身份进入

两条学习路线均由阶段目标、必读资料与动手任务组成,网站以**模拟 IC 设计为主线**。
...
!!! tip "模拟 IC 设计工程师"
    ...
    - **[:material-chip: 进入模拟 IC 路线](learn/analog-ic.md)**
...
| 板块 | 是什么 | 入口 |
|------|--------|------|
| 学习路线 | 按读者身份规划的分阶段路径(模拟 IC 主线、嵌入式辅助) | [学习路线总览](learn/index.md) |
| 设计参考 | 收发器各模块的参数设计要点(输出级/接收器/SIC 控制/振铃抑制/ESD/电源唤醒)与自查清单 | [设计参考总览](design/index.md) |
| 知识库 | 按协议对象分域的权威知识点:协议、位定时、物理层、收发器设计、控制器、工具、项目实践、SIC 专题 | [知识库总览](knowledge/index.md) |
| 教程 | 原创动手教程:帧结构、SocketCAN 回环、位定时配置、示波器抓帧、plugfest 等 | [教程列表](tutorials/index.md) |
| 资源库 | 标准、专利、论文、期刊、教材、厂商资料、工具与社区的分类索引,含全文查阅 | [资源库总览](resources/index.md) |
| 术语表 | A-Z 词条即时查漏:遇到陌生词点进来先看定义 | [术语表索引](glossary/index.md) |
| 勘误与贡献 | 资料勘误记录,以及提交新资料/勘误的渠道 | [勘误与贡献](contribute.md) |
...
- **想系统浏览**:从[资源库总览](resources/index.md)按类型逐页翻,每条目都标注了来源与是否可免费获取;
...
- 【设计参考上线】面向模拟 IC 工程师新增 10 页收发器**参数设计要点**:参数集规格基线、时序链路预算、输出级、接收比较器、SIC 控制逻辑、振铃抑制电路、ESD 与总线保护、电源唤醒、全模块自查总表
- 【站点重构】全站图表重制:22 张原创 SVG 技术图补齐(修复 SIC 学习笔记裂图),100+ Mermaid 图统一中文字体与深浅色适配,构建门禁修复
...
- **SIC 设计专题上线**:知识库新增[SIC 设计专题](knowledge/sic-design/index.md)(原理→设计→测试三篇深度知识),资源库新增[SIC 设计专题分类](resources/sic-design.md),并开放[资料全文查阅](resources/full-text/index.md)
